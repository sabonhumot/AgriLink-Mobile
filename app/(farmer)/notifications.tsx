...
import { mockFarmerNotifications } from "@/data/farmerData";

export default function FarmerNotificationsScreen() {
  const { top } = useSafeAreaInsets();
  const [notifications, setNotifications] = useState(mockFarmerNotifications);
  ...
  const getNotifStyle = (title: string) => {
    if (title.includes("Order Received"))
      return { icon: "cart" as const, color: Colors.primary };
    if (title.includes("Delivered"))
      return { icon: "car" as const, color: "#3B82F6" };
    if (title.includes("Meeting"))
      return { icon: "people" as const, color: "#8B5CF6" };
    if (title.includes("Price"))
      return { icon: "trending-up" as const, color: "#F59E0B" };
      ...
      >
        {notifications.length > 0 ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyDesc}>No new alerts</Text>
          </View>
        )}
      </ScrollView>
    </View>
